...
class SearchResultResponse(_Strict):
    slug: str
    title: str
    pageType: str
    score: float
    source: str
    snippet: str = ""
    vecRank: int | None = None
    ftsRank: int | None = None


@wiki_router.get("/search", response_model=list[SearchResultResponse])
async def search(
    q: str = Query(..., description="Search query string."),
    workspacePath: str = Query(..., description="Absolute path to a workspace directory."),
    topK: int = Query(10, description="Maximum number of results."),
    pageType: str | None = Query(None, description="Filter by page type: paper or concept."),
) -> list[SearchResultResponse]:
    """Semantic search across wiki pages using hybrid vector + FTS5 + RRF."""
    from xreadagent.wiki.search import semantic_search

    workspace = _open_workspace(workspacePath)
    try:
        results = semantic_search(q, workspace, top_k=topK, page_type=pageType)
...
@wiki_router.post("/reindex", response_model=ReindexResponse)
async def reindex(
    workspacePath: str = Query(..., description="Absolute path to a workspace directory."),
    ftsOnly: bool = Query(False, description="Skip embedding, rebuild only FTS5 index."),
) -> ReindexResponse:
    """Rebuild the vector search index for a workspace."""
    workspace = _open_workspace(workspacePath)
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="sqlite-vec is not installed. Install via: pip install sqlite-vec",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=500, detail=f"failed to open vec.sqlite: {exc}"
        ) from exc
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    "Use ftsOnly=true for FTS5-only rebuild."
                ),
            ) from exc
        except Exception as exc:
            raise HTTPException(
                status_code=500, detail=f"failed to initialize embedder: {exc}"
            ) from exc
    ...
    try:
        ...
        store.close()
        raise HTTPException(
            status_code=500, detail=f"rebuild failed: {exc}"
        ) from exc

    store.close()

    return ReindexResponse(
        papersIndexed=stats["papers"],
        conceptsIndexed=stats["concepts"],
        errors=stats["errors"],
        mode="fts-only" if ftsOnly else "hybrid",
    )
